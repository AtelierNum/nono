...
const evalAnswer = (answer, question) => {
	...
	switch(question.type){
		...
		case "string":
			return levenshteinEditDistance.get(answer.toLowerCase(), goodAnswer.toLowerCase()) <= goodAnswer.length * .25;
		case "number":
			return  answer > goodAnswer - goodAnswer * .001 && answer < goodAnswer + goodAnswer * .001
	}
}

const routine = (ctx) => {
    clearTimeout(timeout);
		timeout = setTimeout(() => handleTimeout(nono,quizzPassiveListener, ctx.msg.channel),timeBeforeQuit)
